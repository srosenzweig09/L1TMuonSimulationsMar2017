...
def is_emtf_singlehit_me23(mode):
  return bool(mode & (1 << 2)) | bool(mode & (1 << 3))
...
class PatternRecognition(object):
  ...
  def _apply_patterns(self, endcap, sector, sector_hits):
    ...
    for road_id, road_hits in amap.iteritems():
      ...
      #FIXME
      if ((is_emtf_muopen(road_mode) and is_emtf_muopen(road_mode_csc)) or \
  ...
  def run(self, hits):
    ...
    for endcap in (-1, +1):
      for sector in (1, 2, 3, 4, 5, 6):
        ...
        # Provide early exit if fail MuOpen and no hit in stations 1&2&3 (check CSC, ME0, DT)
        if not is_emtf_muopen(sector_mode) and \
            not is_emtf_singlehit(sector_mode) and \
            not is_emtf_singlehit_me23(sector_mode):
          continue
...
class MixingAnalysis(object):
  def run(self, omtf_input=False, run2_input=False, test_job=159):
    ...
    if training_phase:
      rng_engines = []
      for rng_seed in xrange(2011, 2011+12):
        rng_engines.append(np.random.RandomState(rng_seed))
    else:
      rng_engines = [None]

    def scramble_bx(rng, hits):
      for hit in hits:
        if rng is not None:
          hit.bx = rng.randint(-3, +4, dtype=np.int32)
    ...
    for ievt, evt in enumerate(tree):
      ...
      # Scramble hit BX multiple times
      for rng_engine in rng_engines:

        # Scramble hit BX
        scramble_bx(rng_engine, evt.hits)
