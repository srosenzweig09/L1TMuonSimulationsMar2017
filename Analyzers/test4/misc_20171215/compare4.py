...
def main(histos, options):
  ...
  hnames = [
      "highest_emtf_absEtaMin0_absEtaMax2.5_qmin12_mctruth1_pt",
      "highest_emtf_absEtaMin0_absEtaMax2.5_qmin12_mctruth2_pt",
      "highest_emtf_absEtaMin0_absEtaMax2.5_qmin12_mctruth3_pt",
  ]
  ...
  for infile in infiles:
    for hname in hnames:
      ...
      hname_ref = "highest_emtf_absEtaMin0_absEtaMax2.5_qmin12_pt"
      tfile = TFile.Open(infile_ref)
      h_nevents = tfile.Get("trackcounting/nevents")
      assert h_nevents != None, "Cannot get nevents"
      nevents = h_nevents.GetBinContent(2)
      print("nevents: {0}".format(nevents))
      h = tfile.Get("trackcounting/"+hname_ref)
      assert h != None, "Cannot get %s" % hname_ref
      h = h.Clone(hname + "_2")
      h.Sumw2()
      make_ptcut(h)
      make_rate(h, nevents)
      ...
      if "mctruth1" in hname:
        imgname += "_mctruth1"
      elif "mctruth2" in hname:
        imgname += "_mctruth2"
      elif "mctruth3" in hname:
        imgname += "_mctruth3"
...
if __name__ == "__main__":
  ...
  parser.add_argument("--outdir", default="plots_compare4/", help="output directory (default: %(default)s)")
